...
class DataManager {
    ...
    async loadMessagesFromFirebase() {
        if (!this.isFirebaseReady || !firebaseManager.isReady()) {
            return;
        }

        try {
            // جلب الرسائل الموجودة
            const existingMessages = await firebaseManager.getExistingMessages();
            this.messageCache = existingMessages;

            // الاستماع للرسائل الجديدة
            firebaseManager.listenForMessages((message) => {
    ...
    async getUsers() {
        if (this.isFirebaseReady && firebaseManager.isReady()) {
            return await firebaseManager.getUsers();
        }
    ...
    async getUserById(id) {
        if (this.isFirebaseReady && firebaseManager.isReady()) {
            return await firebaseManager.getUserById(id);
        }
        return null;
    }
    ...
    async getUserByUsername(username) {
        if (this.isFirebaseReady && firebaseManager.isReady()) {
            return await firebaseManager.getUserByUsername(username);
        }
        return null;
    }
    ...
    async addUser(userData) {
        if (this.isFirebaseReady && firebaseManager.isReady()) {
            ...
            const addedUser = await firebaseManager.addUser(newUser);
            return addedUser;
        }
    ...
    async updateUserActivity(userId) {
        if (this.isFirebaseReady && firebaseManager.isReady()) {
            await firebaseManager.logUserActivity(userId, 'activity_update');
        }
    }
    ...
    async getPosts() {
        if (this.isFirebaseReady && firebaseManager.isReady()) {
            return await firebaseManager.getPosts();
        }
        return [];
    }
    ...
    async addPost(postData) {
        if (this.isFirebaseReady && firebaseManager.isReady()) {
            ...
            const addedPost = await firebaseManager.savePost(newPost);
            return addedPost;
        }
    ...
    async togglePostLike(postId, userId) {
        if (this.isFirebaseReady && firebaseManager.isReady()) {
            await firebaseManager.togglePostLike(postId, userId);
    ...
    async addMessage(messageData) {
        if (this.isFirebaseReady && firebaseManager.isReady()) {
            ...
            const addedMessage = await firebaseManager.sendMessage(newMessage);
            return addedMessage;
        }
    ...
    async getActiveStories() {
        if (this.isFirebaseReady && firebaseManager.isReady()) {
            return await firebaseManager.getActiveStories();
        }
        return [];
    }
    ...
    async addStory(storyData) {
        if (this.isFirebaseReady && firebaseManager.isReady()) {
            ...
            const addedStory = await firebaseManager.saveStory(newStory);
            return addedStory;
        }
    ...
    async getNovels() {
        if (this.isFirebaseReady && firebaseManager.isReady()) {
            return await firebaseManager.getNovels();
        }
        return [];
    }
    ...
    async addNovel(novelData) {
        if (this.isFirebaseReady && firebaseManager.isReady()) {
            ...
            const addedNovel = await firebaseManager.saveNovel(newNovel);
            return addedNovel;
        }
    ...
    async getSettings() {
        if (this.isFirebaseReady && firebaseManager.isReady()) {
            return await firebaseManager.getSettings();
        }
    ...
    async updateSettings(newSettings) {
        if (this.isFirebaseReady && firebaseManager.isReady()) {
            await firebaseManager.saveSettings(newSettings);
    ...
    async getScreenshots() {
        if (this.isFirebaseReady && firebaseManager.isReady()) {
            return await firebaseManager.getScreenshots();
        }
        return [];
    }
    ...
    async addScreenshot(userId, page) {
        if (this.isFirebaseReady && firebaseManager.isReady()) {
            ...
            const addedScreenshot = await firebaseManager.saveScreenshot(newScreenshot);
