...
class AuthManager {
    ...
    async syncUserDataWithFirebase() {
        if (!firebaseManager.isReady() || !this.currentUser) {
            return;
        }

        try {
            // حفظ بيانات المستخدم الحالي في Firebase
            await firebaseManager.saveUserData(this.currentUser.id, {
    ...
    async saveSession(user) {
        ...
        if (firebaseManager.isReady()) {
            try {
                await firebaseManager.saveUserData(user.id, {
                    ...sessionData.user,
                    lastLogin: Date.now(),
                    sessionActive: true
                });
    ...
    // مسح الجلسة
    async clearSession() {
        if (this.currentUser && firebaseManager.isReady()) {
            try {
    ...
    verifyAdminCode(code) {
        // يجب جلب الإعدادات من Firebase الآن
        // هذه الوظيفة قد تحتاج إلى أن تكون async إذا كانت تعتمد على جلب الإعدادات من Firebase
        // مؤقتاً، سنفترض أن الإعدادات متاحة أو سيتم جلبها بشكل آخر
        return code === "Wwsdjehadadmen56780097gg"; // الرمز الثابت للآدمن
    }
    ...
    async login(username, password) {
        try {
            ...
            const user = await firebaseManager.getUserByUsername(username);
            if (!user) {
            ...
            if (hashedPassword !== user.passwordHash) { // تأكد من استخدام passwordHash
            ...
            if (firebaseManager.isReady()) {
                try {
                    await firebaseManager.logUserActivity(user.id, 'login');
                } catch (error) {
    ...
    async register(username, password, avatar = null) {
        try {
            ...
            const existingUser = await firebaseManager.getUserByUsername(username);
            if (existingUser) {
            ...
            const newUser = await firebaseManager.addUser({
                username,
                passwordHash: hashedPassword, // تأكد من استخدام passwordHash
            ...
            if (firebaseManager.isReady()) {
                try {
                    await firebaseManager.logUserActivity(newUser.id, 'register');
                } catch (error) {
    ...
    async logout() {
        ...
        if (this.currentUser && firebaseManager.isReady()) {
            try {
                await firebaseManager.logUserActivity(this.currentUser.id, 'logout');
            } catch (error) {
    ...
    async enableLock(pin) {
        ...
        if (this.currentUser && firebaseManager.isReady()) {
            try {
                await firebaseManager.logUserActivity(this.currentUser.id, 'lock_enabled');
            } catch (error) {
    ...
    async disableLock() {
        ...
        if (this.currentUser && firebaseManager.isReady()) {
            try {
                await firebaseManager.logUserActivity(this.currentUser.id, 'lock_disabled');
            }
        }
    ...
    async lockApp() {
        ...
        if (settings && settings.lockEnabled) {
            ...
            if (this.currentUser && firebaseManager.isReady()) {
                try {
                    await firebaseManager.logUserActivity(this.currentUser.id, 'app_locked');
                }
            }
    ...
    async unlockApp(pin) {
        ...
        if (hashedPin === settings.lockPin) {
            ...
            if (this.currentUser && firebaseManager.isReady()) {
                try {
                    await firebaseManager.logUserActivity(this.currentUser.id, 'app_unlocked');
                }
            }
    ...
    showLockOverlay() {
        ...
        // إضافة معالج حدث لنموذج إلغاء القفل
        const unlockForm = document.getElementById('unlockForm');
        if (unlockForm) {
            unlockForm.onsubmit = async (e) => {
                e.preventDefault();
                const unlockPin = document.getElementById('unlockPin').value;
                const unlockError = document.getElementById('unlockError');
                const result = await this.unlockApp(unlockPin);
                if (!result.success) {
                    unlockError.textContent = result.error;
